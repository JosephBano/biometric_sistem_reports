"""
Servicio de narrativas IA (`app.domain.ai_narrative`).

Genera explicaciones textuales legibles sobre los hallazgos de
`app.domain.analytics`. Utiliza la API de DeepSeek si la clave está
disponible; de lo contrario, usa un motor regla-base determinista.

Migrado desde `ia_report.py` (raíz) en Fase 4e.4 del ADR-0001.
...
def generar_narrativo(hallazgos: dict, contexto: str = "") -> str:
    """
    Toma un diccionario de hallazgos estadísticos de analytics y devuelve
    una narrativa explicativa. Usa DeepSeek si la clave está disponible; si
    no, motor regla-base.
    """
    ...
    riesgos_altos = [
        r for r in hallazgos.get("riesgos", []) if r.get("semaforo") == "Rojo"
    ]
    ...
    if tasa >= 90:
        ...
        text += (
            "El nivel es **aceptable** pero se detectan oportunidades de mejora "
            "en puntualidad.\n\n"
        )
    else:
        text += (
            "La tasa está **por debajo del estándar mínimo (75%)** y requiere "
    ...
    if riesgos_altos:
        ...
        text += (
            "No se identificaron personas en zona de riesgo alto. "
            "El equipo muestra un cumplimiento consistente.\n\n"
        )
    ...
    if ausentes > 0:
        ...
        if pct_aus > 10:
            text += (
                "Verificar justificaciones y evaluar medidas de incentivo "
                "a la asistencia.\n"
            )
        else:
