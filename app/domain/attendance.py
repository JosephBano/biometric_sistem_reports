"""
Servicio de horarios (`app.domain.attendance`).

Parseo de archivos de horarios (`.csv` y `.obd` del reloj ZK), normalización
de horas y almuerzos, y resolución del horario aplicable para una persona
en una fecha concreta.

Migrado desde `horarios.py` (raíz) en Fase 4e.5 del ADR-0001.

API pública:
  - `parsear_csv(filepath) -> list[dict]`
  - `parsear_obd(filepath) -> list[dict]`
  - `get_info_dia(horario_persona, fecha) -> dict`
"""
...
__all__ = ["parsear_csv", "parsear_obd", "get_info_dia"]
