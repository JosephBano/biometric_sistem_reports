"""
Motor de generación de reportes PDF (`app.domain.reports`).

Funciones públicas (también re-exportadas para compatibilidad con blueprints):
  - `DEFAULT_CONFIG`
  - `cargar_archivo(ruta) -> list[dict]`
  - `filtrar_excluidos(registros, excluidos) -> list`
  - `deduplicar(registros, duplicado_min=0.5) -> tuple[list, log]`
  - `analizar_dia(registros, horarios, ...) -> dict`
  - `analizar_por_persona(registros, config, ...) -> dict`
  - `generar_pdf(...)` y `generar_pdf_persona(...)`

Migrado desde `script.py` (raíz, 2 281 LOC) en Fase 4e.1 del ADR-0001.
"""
...
# DEFAULT_CONFIG, analizar_dia, etc. ahora viven en este mismo módulo
# (migrados desde script.py). Ya no hace falta re-export.
