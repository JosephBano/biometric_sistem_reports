"""
Motor unificado de Analytics (`app.domain.analytics`).
...
Migrado desde `analytics.py` (raíz) en Fase 4e.3 del ADR-0001.

API pública:
  - `load_data_asistencia_dataframe(fecha_inicio, fecha_fin, grupo_id, tipo_persona_id)`
  - `calcular_risk_score(persona_df)`
  - `analizar(tipo_persona_id, grupo_id, periodo_vigencia_id, fecha_inicio, fecha_fin)`
  - `patron_semanal(persona_id, semanas)`
  - `comparar_grupos(grupo_ids, fecha_inicio, fecha_fin)`
  - `ranking_departamento(grupo_id, fecha_inicio, fecha_fin)`
  - `tendencia_mensual(tipo_persona_id, meses)`
  - `resumen_periodo(periodo_id)`
  - `distribucion_asistencia_periodo(periodo_id)`
  - `comparar_periodos_historicos(tipo_persona_id, n)`
  - `tasa_riesgo_por_grupo(grupo_id)`
"""
...
__all__ = [
    "load_data_asistencia_dataframe",
    "calcular_risk_score",
    "analizar",
    "patron_semanal",
    "comparar_grupos",
    ...
    "comparar_periodos_historicos",
    "tasa_riesgo_por_grupo",
]
