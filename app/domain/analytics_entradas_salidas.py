...
from app.domain.asistencia_dia import resumir_rango
...
# Un mes entra completo en una página del resumen diario.
DIAS_POR_PAGINA = 31


def _validar_consulta(persona_id: str, fecha_inicio: date, fecha_fin: date) -> None:
    """
    Validaciones comunes a las dos consultas. Lanza ValueError; el blueprint
    lo traduce a un 400 con el mensaje.
    """
    if not persona_id:
...
def consultar_resumen_diario(
    ...
    """
    Resumen por día del rango: primer/último marcaje, permanencia, tiempo
    efectivo, cantidad de marcajes y estado del marcaje.

    Se pagina por día (no por marcación), porque la fila de la tabla es el día.
    Cada fila trae sus marcaciones embebidas para el popup de detalle.

    Returns:
      {
        "persona_id": str, "fecha_inicio": str, "fecha_fin": str,
        "page": int, "total_dias": int, "total_pages": int,
        "dias": list[dict],
      }
    """
    _validar_consulta(persona_id, fecha_inicio, fecha_fin)
    if page < 1:
        page = 1

    marcaciones = listar_marcaciones_rango(persona_id, fecha_inicio, fecha_fin, schema=schema)
    filas = resumir_rango(fecha_inicio, fecha_fin, marcaciones)

    total_dias = len(filas)
    total_pages = (total_dias + DIAS_POR_PAGINA - 1) // DIAS_POR_PAGINA
    inicio = (page - 1) * DIAS_POR_PAGINA
    ...
    return {
        ...
        "page": page,
        "per_page": DIAS_POR_PAGINA,
        "total_dias": total_dias,
        "total_pages": total_pages,
        "dias": filas[inicio:inicio + DIAS_POR_PAGINA],
    }
...
__all__ = ["consultar_entradas_salidas", "consultar_resumen_diario"]